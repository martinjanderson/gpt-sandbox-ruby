...
# A class that encapsulates this Sinatra application
class AlbumApp < Sinatra::Base

    # Enforce SSL only for non local and non test environments 
    use Rack::SslEnforcer if ENV['RACK_ENV'] != 'development' && ENV['RACK_ENV'] != 'test'
